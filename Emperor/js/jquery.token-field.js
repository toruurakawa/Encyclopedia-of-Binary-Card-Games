...
(function($) {
  $.fn.tokenField = function(options) {
          ...
          if (!$(this).data('blur')) {
            ...
            if (settings.max == 0 || $(this).closest('.token-input').siblings('.token').length < settings.max) {
              if (isToken($(this).val())) {
                ...
                var inArray = isInArray($(this).val(), letterArray);
                var property;
                if ( inArray === true) {
                  property = $(this).val();
                }

                observeToken($(tokenHtml($(this).attr('name'), $(this).val(), property)).insertBefore($(this).closest('.token-input')));
                if (!settings.nested) {
    ...
    function tokenHtml(name,text, property, image) {
      // alert(property);
      return "<a href='#' class='token'><span><span><span><span class='image-span-wrapper'><div class='image-span " + property + "'></div>"
